...
def move_motor_pwm(steps, direction, speed=500):
    ...
    # This is a simple calculation: duration = number of steps / (frequency * steps per cycle)
    # Assuming one pulse per step for simplicity. Adjust as necessary for your setup.
    steps_per_cycle = 1  # For full step. Adjust if using microstepping.
    duration = steps / (speed * steps_per_cycle)
    
    # Enable PWM
    pi.set_PWM_frequency(STEP_PIN, speed)  # Set the speed
...
def main_code():
    ...
    try:
        ...
        # Stop the motor by stopping the PWM signal on the STEP pin
